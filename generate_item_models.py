...
etc_texture_folder = r"MapleCraft resource pack\assets\minecraft\textures\item\etc"
etc_model_folder = r"MapleCraft resource pack\assets\minecraft\models\item\etc"

try:
	for folder in os.listdir(etc_texture_folder):
		texture_parent_path = os.path.join(etc_texture_folder, folder)
		if os.path.isdir(texture_parent_path):
			model_parent_path = os.path.join(etc_model_folder, folder)
			...
			for file in os.listdir(texture_parent_path):
				...
				with open(os.path.join(model_parent_path, file + ".json"), "w") as f:
					model = {
						...
						"textures": {
							"layer0": f"minecraft:item/etc/{folder}/{file}"
						},
						"display": {
							"ground": {
								"rotation": [ 0, 0, 0 ],
